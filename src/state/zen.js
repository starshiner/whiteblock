...
**/
;var gameObj = gameObj || {};
gameObj.State_zen = function(){
    this.preload = function(){
        this.game.stage.backgroundColor = "#fff";
    };
    this.create = function(){
        //定义游戏基本属性
        gameObj.gameMode = "禅模式";
        gameObj.gameModeCode = "zen";
        gameObj.musicNum = 0;
        gameObj.score = 0;
        ...
        this.blockLine = 7;
        this.endTime = 10000;
        this.lable_score = this.game.add.text(gameObj.gameWidth/2, 30, 0 , {font: "45px Arial",fill: "#f00"});
        this.lable_score.anchor.set(0.5);
        this.lable_rate = this.game.add.sprite(0, 0, 'rate_line');
        this.lable_rate.scale.x = 1;
    ...
    this.update = function(){    
        ...
        if(this.onStartGame){
            ...
            this.lable_rate.scale.x = (this.endTime-(this.game.time.prevTime-this.startTime))/this.endTime;
    ...
    this.clickBlack = function(){
        ...
        console.log((gy - 526)%131);
        if((gy - 526)%131 != 0){
            var gy = (Math.floor((gy-526)/131)+1)*131+526;
        }
        var tmpy = gy + 131;
        console.log("++++++"+(tmpy - 526)%131);
        this.game.add.tween(this.ground).to( {  y : tmpy }, 200, null, true, 0, 0, false);
